...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  namecheap-pp-cli doctor
  namecheap-pp-cli doctor --json
  namecheap-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if strings.TrimSpace(cfg.APIUser) == "" || strings.TrimSpace(cfg.APIKey) == "" {
					...
					report["auth_hint"] = "export NAMECHEAP_USERNAME=<user> NAMECHEAP_API_KEY=<key>; NAMECHEAP_CLIENT_IP must be whitelisted by Namecheap"
				} else {
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					if strings.TrimSpace(cfg.ClientIP) == "" {
						report["client_ip"] = "auto-detect"
					} else {
						report["client_ip"] = "configured"
					}
				}
			}
			...
			if os.Getenv("NAMECHEAP_USERNAME") != "" || os.Getenv("NAMECHEAP_API_USER") != "" {
				authEnvSet = append(authEnvSet, "NAMECHEAP_USERNAME")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NAMECHEAP_USERNAME")
			}
			if os.Getenv("NAMECHEAP_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "NAMECHEAP_API_KEY")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NAMECHEAP_API_KEY")
			}
			if os.Getenv("NAMECHEAP_CLIENT_IP") != "" {
				authEnvSet = append(authEnvSet, "NAMECHEAP_CLIENT_IP")
			}
			if len(authEnvRequiredMissing) > 0 {
				report["env_vars"] = "ERROR missing required: " + strings.Join(authEnvRequiredMissing, ", ")
			} else {
				report["env_vars"] = fmt.Sprintf("OK required set (%d vars available)", len(authEnvSet))
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Step 2: Validate credentials with a safe authenticated Namecheap API probe.
					if strings.TrimSpace(cfg.APIUser) == "" || strings.TrimSpace(cfg.APIKey) == "" {
						...
						case errors.As(authErr, &authAPIErr):
							if authAPIErr.StatusCode == 401 || authAPIErr.StatusCode == 403 {
								...
								report["credentials"] = fmt.Sprintf("inconclusive (HTTP %d from Namecheap)", authAPIErr.StatusCode)
							}
						default:
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("namecheap-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'namecheap-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'namecheap-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'namecheap-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'namecheap-pp-cli sync' to refresh."
	}
	return report
}
